...
!      integer(kind=kint),allocatable,dimension(:,:,:) :: kk2,IDlu2
      integer(kind=kint) :: kk3(8,8),IDlu3(8,8),ipp(8)
...
!$acc enter data
!$acc& create(AL,AU,ALUG,D,B,X,WW,WS,WR,aa)
...
!$acc parallel vector_length(64)
!$acc loop independent gang private(kk3,IDlu3,ipp)
      do icel0=ELMCOLORindex(icol-1)+1,ELMCOLORindex(icol)
!C
!C== CONSTRUCT the GLOBAL MATRIX
